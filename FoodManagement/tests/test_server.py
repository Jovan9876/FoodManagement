"""
This module contains tests for the Flask server of the Food Management system.

The tests ensure that the following functionalities work as expected:
- User authentication (register, login, logout, and fetch current user).
- Food inventory management (add, update, and retrieve food items).
- Notifications for low-stock food items.
- Shopping list management (save and retrieve shopping lists).
"""
...
def test_logout_user(client):
    ...
    )  # Should return unauthorized as the session should be cleared
...
def test_get_inventory(client):
    ...
    inventory = [dict(zip(headers, item)) for item in items]
